...
class HistoryActivity : AppCompatActivity() {
    ...
    private fun initialModule() {
        ...
        retrofit = Retrofit.Builder()
            ...
            .baseUrl("http://10.51.183.60/tree_store/")
            .build()
    }
    ...
    private fun getHistory() {
        ...
        disposable = service.getUserPayment(mAuth.data.id)
...
class HistoryAdapter(private val list: List<PaymentModel.Data>) :
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        view.historyItemList.setOnClickListener {
            val intent = Intent(view.context, HistoryDetailActivity::class.java).apply {
                putExtra("id", item.payment_id)
            }
            view.context.startActivity(intent)
        }
        Glide.with(view.context).load("http://10.51.183.60/tree_store/uploads/${item.picture}")
